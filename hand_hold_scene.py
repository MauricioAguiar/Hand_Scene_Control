...
move_sceneitem_scale = obs.vec2()  # Initial rotation angle, used as well for oscillations
...
def move_source():
  ...
  if sceneitem:
    font_id = obs.obs_sceneitem_get_id(sceneitem)
    ...
    video_capture(sceneitem)
    #angle = move_sceneitem_scale + move_pos_trheshold*math.sin(time.time()*move_scale_threshold*2*math.pi)
...
def script_defaults(settings):
  global move_sceneitem_scale
  move_sceneitem_scale = obs.vec2()
...
def script_update(settings):
  global source_name, move_scale_threshold, move_pos_trheshold, debug
  restore_sceneitem_after_move()
...
def moving_source(coords):  
  ...
  local_scale.x = (scale.x*0.1)* smooth_differ(coords[2], move_scale_threshold, "scale_x")
  local_scale.y = (scale.y*0.1)* smooth_differ(coords[2], move_scale_threshold, "scale_y")
  pos.x = smooth_differ(coords[0], move_pos_trheshold, "pos_x")
  pos.y = smooth_differ(coords[1], move_pos_trheshold, "pos_y")
  obs.obs_sceneitem_set_pos(sceneitem, pos)
  obs.obs_sceneitem_set_scale(sceneitem, local_scale)
